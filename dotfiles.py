...
class Config:
    ...
    def load(cls, config_path: Path = None) -> "Config":
        ...
        try:
            with open(config_path) as f:
                if config_path.suffix == ".json":
                    ...
                    out.error(
                        f"Cannot parse {config_path.suffix} files: PyYAML not installed"
                    )
        ...
        return cls(
            ...
            scan_paths=[
                Path(p).expanduser()
                for p in data.get("scan_paths", ["~/.config", "~/"])
            ],
        ...
        for base, group in groups.items():
            ...
            if os_match:
                ...
                generic = [
                    f
                    for f in group
                    if not any(str(f).endswith(s) for s in config.os_patterns)
                ]
                if generic:
...
class FileManager:
    ...
    def create_symlink(
        self, target: Path, link: Path, dry_run: bool = False
    ) -> Tuple[bool, str]:
        ...
        try:
            link_resolved = link.resolve()
            if (
                self.config.repo_root in link_resolved.parents
                or link_resolved == self.config.repo_root
            ):
                return False, "would create circular symlink"
        except:
            pass
        ...
        if dry_run:
            return (
                True,
                f"would link (backup: {backup_path.name if backup_path else 'none'})",
            )
        ...
        try:
            ...
            return (
                True,
                f"linked (backup: {backup_path.name if backup_path else 'none'})",
            )
        except OSError as e:
            return False, str(e)

    def remove_symlink(
        self, link: Path, restore_backup: bool = False
    ) -> Tuple[bool, str]:
    ...
    def move_to_repo(
        self, source: Path, repo_path: Path, dry_run: bool = False
    ) -> Tuple[bool, Path, str]:
    ...
    def _is_tracked(self, path: Path) -> bool:
        ...
        if path.is_symlink():
            try:
                target = path.resolve()
                return (
                    self.config.repo_root in target.parents
                    or target == self.config.repo_root
                )
            except:
                return False
...
class DotfilesManager:
    ...
    def cmd_link(self, dry_run: bool = False):
        ...
        for rel_path, repo_dir in config_dirs:
            ...
            success, message = self.file_manager.create_symlink(
                repo_dir, home_path, dry_run
            )
    ...
    def cmd_scan(self):
        ...
        if untracked:
            ...
            if auto_matches:
                out.print(
                    f"\n[bold green]Suggested for import (auto-detected):[/bold green]"
                    if HAS_RICH
        ...
        out.print(
            f"\n[dim]Use './dotfiles.py import <numbers>' to import selected items[/dim]"
            if HAS_RICH
    ...
    def cmd_import(
        ...
        if paths:
            ...
            for i, item in enumerate(untracked, 1):
                icon = "📁" if item.is_dir else "📄"
                is_suggested = any(
                    pattern in item.name for pattern in self.config.auto_import
                )
    ...
    def cmd_status(self):
        ...
        if broken > 0:
            out.error(
                f"Found {broken} broken symlinks. Run './dotfiles.py link' to fix."
            )
...
Examples:
  ...
  %(prog)s uninstall               # Remove symlinks and restore backups
    ...
    link_parser = subparsers.add_parser(
        "link", help="Create symlinks from repo to home"
    )
    ...
    scan_parser = subparsers.add_parser(
        "scan", help="Find untracked configuration files"
    )

    # Import command
    import_parser = subparsers.add_parser(
        "import", help="Import configs to repo and symlink"
    )
    import_parser.add_argument(
        "selections", nargs="*", type=int, help="Item numbers from scan to import"
    )
    import_parser.add_argument("--path", "-p", nargs="+", help="Import specific paths")
    import_parser.add_argument(
        "--dry-run",
        "-n",
        action="store_true",
    ...
    uninstall_parser = subparsers.add_parser(
        "uninstall", help="Remove symlinks and restore backups"
    )
