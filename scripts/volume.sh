...
# credits to dastorm: https://github.com/dastorm/volume-notification-dunst/blob/master/volume.sh
...
case $1 in
	...
	if is_mute ; then
    DIR=`dirname "$0"`
    $DIR/notify-send.sh -i "/usr/share/icons/Papirus-Light/48x48/status/notification-audio-volume-muted.svg" --replace=555 -u normal "Mute" -t 2000
	else
	    send_notification
	fi
	;;
